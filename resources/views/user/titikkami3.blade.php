...
@section('morejs')
    ...
    <script src="{{ asset('js/map-control7.js') }}?v=3"></script>
    ...
    <script>
        ...
        window.generateGoogleMapData = async function() {
            try {
                ...
                // Menghapus limit agar semua data tampil, tapi akan menggunakan MarkerClusterer
                removeMultiMarker?.();
                if (payload.length > 0) {
        ...
        function forceShowAllMarkers() {
            ...
            // Jika ada MarkerClusterer, gunakan clusterer agar peta tidak berat (N+1 DOM issues)
            if (window.markerCluster) {
                window.markerCluster.clearMarkers();
                window.markerCluster.addMarkers(window.multi_marker || []);
            } else {
                (window.multi_marker || []).forEach(m => m?.setMap?.(mapObj));
            }
            (window.ACTIVE_MARKERS || []).forEach(m => {
                if (!window.markerCluster) m?.setMap?.(mapObj);
            });
        }

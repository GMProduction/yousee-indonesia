...
<body class="flex h-screen overflow-hidden text-slate-800">
    ...
    <main class="flex-1 flex flex-col overflow-y-auto bg-[#F8FAFC]">
        ...
        <div id="view-dashboard" class="view-section active p-8 max-w-[1600px] mx-auto w-full fade-in space-y-8">
            ...
            <div id="analysis-panel" class="grid grid-cols-1 md:grid-cols-3 gap-6 fade-in" style="display: none;">
                ...
                <div
                    ...
                    <p class="text-xs text-indigo-100 mt-4 bg-white/10 inline-block px-2 py-1 rounded">Lokasi Sangat
                        Strategis</p>
                </div>
                <!-- Card 3: Audience -->
        ...
        <div id="view-map" class="view-section h-full relative fade-in">
            ...
            <div
                class="absolute top-3 left-[180px] right-14 z-[550] flex flex-col md:flex-row items-start md:items-center gap-2 pointer-events-none max-w-full overflow-x-auto no-scrollbar pr-2">
                ...
                <div class="relative group pointer-events-auto rounded-lg shadow-sm min-w-[280px] md:w-[320px]">
                    ...
                    <input type="text" id="map-search-input" onkeyup="handleMapSearch()"
                        ...
                        placeholder="Cari lokasi, jalan..." autocomplete="off">

                    <!-- Clear Button -->
                    <button id="map-search-clear" onclick="clearMapSearch()"
                        class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-300 hover:text-red-500 hidden cursor-pointer transition-colors">
                        <i class="fa-solid fa-circle-xmark"></i>
                    </button>

                    <!-- Results Dropdown -->
                    <div id="map-search-results"
                        class="hidden absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-[0_10px_40px_rgb(0,0,0,0.12)] overflow-hidden border border-slate-100 max-h-[400px] overflow-y-auto custom-scrollbar pointer-events-auto z-[1000]">
                        <!-- Results List -->
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 pointer-events-auto pb-1 md:pb-0">
                    ...
                    <div class="relative group rounded-lg bg-white/95 backdrop-blur-sm shadow-sm">
                        ...
                        <select id="filter-map-province" onchange="handleMapFilterChange()"
                            class="block w-[140px] pl-8 pr-6 py-2.5 border-0 rounded-lg bg-transparent text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer appearance-none truncate">
                    ...
                    <div class="relative group rounded-lg bg-white/95 backdrop-blur-sm shadow-sm">
                        ...
                        <select id="filter-map-city" onchange="handleMapFilterChange()"
                            class="block w-[130px] pl-8 pr-6 py-2.5 border-0 rounded-lg bg-transparent text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer appearance-none truncate disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div class="relative group rounded-lg bg-white/95 backdrop-blur-sm shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                            <i
                                class="fa-solid fa-layer-group text-slate-400 group-focus-within:text-blue-500 text-xs"></i>
                        </div>
                        <select id="filter-map-type" onchange="handleMapFilterChange()"
                            class="block w-[130px] pl-8 pr-6 py-2.5 border-0 rounded-lg bg-transparent text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer appearance-none truncate">
                    ...
                    <div class="relative group rounded-lg bg-white/95 backdrop-blur-sm shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                            <i
                                class="fa-solid fa-arrows-up-down-left-right text-slate-400 group-focus-within:text-blue-500 text-xs"></i>
                        </div>
                        <select id="filter-map-position" onchange="handleMapFilterChange()"
                            class="block w-[130px] pl-8 pr-6 py-2.5 border-0 rounded-lg bg-transparent text-xs font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer appearance-none truncate">
        ...
        <div id="floating-search-results"
            style="display:none; position:fixed; top:140px; left:200px; width:400px; max-height:500px; background:white; border-radius:16px; box-shadow:0 20px 60px rgba(0,0,0,0.2); z-index:10000; overflow-y:auto; border:2px solid #3b82f6;">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-indigo-50 p-3 rounded-xl border border-indigo-100">
                        <span
                            class="text-[10px] text-indigo-600 font-bold uppercase block mb-1">{{ __('geospasial.kpi.ai_score') }}</span>
        ...
        <div id="view-analysis" class="view-section p-6 max-w-[1600px] mx-auto w-full fade-in pb-20">
            ...
            <button onclick="switchView('map')"
                class="mb-6 flex items-center gap-2 text-slate-500 hover:text-slate-800 transition font-medium text-sm">
            ...
            <div class="flex flex-wrap justify-center gap-6 pb-20 items-stretch">
                ...
                <div
                    class="bg-white rounded-[2rem] shadow-xl border border-slate-200 p-6 w-full lg:w-[48%] max-w-[700px] flex flex-col relative overflow-hidden h-full">

                    <!-- DOWNLOAD BUTTON: Left -->
                    <button onclick="downloadVisual()" data-html2canvas-ignore="true"
                        class="absolute top-6 right-6 z-50 bg-black hover:bg-slate-800 text-white text-[10px] font-bold py-2 px-3 rounded-lg transition shadow-lg flex items-center gap-2">
                        <i class="fa-solid fa-download"></i> Download Image
                    </button>

                    <!-- Main Image (SCROLLABLE HEIGHT) -->
                    <div
                        class="relative w-full h-full bg-slate-50 rounded-lg overflow-hidden border border-slate-100 mt-8 overflow-y-auto custom-scrollbar">
                        <img id="poster-image" src="" class="w-full h-auto object-cover">
                    </div>
                </div>

                <!-- 2. RIGHT POSTER: LOCATION INTELLIGENCE (A4 Landscape) -->
                <div id="right-poster-container"
                    class="bg-white rounded-[2rem] shadow-xl border border-slate-200 p-6 lg:p-8 w-full lg:w-[48%] max-w-[700px] relative overflow-hidden flex flex-col h-full">

                    <!-- DOWNLOAD BUTTON: Right (Ignored by html2canvas) -->
                    <button onclick="downloadData()" data-html2canvas-ignore="true"
                        class="absolute top-6 right-6 z-50 bg-black hover:bg-slate-800 text-white text-[10px] font-bold py-2 px-3 rounded-lg transition shadow-lg flex items-center gap-2">
                        <i class="fa-solid fa-download"></i> Download Image
                    </button>
                    ...
                    <div
                        class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400">
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
                        ...
                        <div class="flex flex-col gap-5">
                            ...
                            <div
                                class="bg-white p-5 rounded-xl border border-purple-100 shadow-sm relative overflow-hidden group hover:border-purple-200 transition h-[160px] flex flex-col justify-center">
                                <div
                                    class="absolute right-0 top-0 w-16 h-16 bg-purple-50 rounded-bl-full -mr-4 -mt-4 transition group-hover:bg-purple-100">
                                </div>
                                <div class="relative z-10">
                                    <div class="flex items-center gap-2 mb-1">
                                        <div
                                            class="w-5 h-5 rounded-full bg-purple-100 flex items-center justify-center text-purple-600">
                                            <i class="fa-solid fa-eye text-[10px]"></i>
                                        </div>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase">AVG.
                                            VIEWS/DAY</span>
                                    </div>
                                    <h2 class="text-4xl font-extrabold text-slate-800 tracking-tight"
                                        id="detail-avg-views">0</h2>
                            ...
                            <div
                                class="bg-white p-5 rounded-xl border border-slate-100 shadow-sm space-y-3 relative group h-[225px] flex flex-col justify-center">
                                <!-- EDIT BUTTON (Moved Inside Tech Specs) -->
                                <button onclick="toggleEditMode()" id="btn-edit-location"
                                    class="absolute top-1 right-4 text-slate-400 hover:text-blue-600 transition p-1 z-10"
                                    title="Edit Data Lokasi">
                                    <i class="fa-solid fa-pen-to-square"></i> <span
                                        class="text-xs font-bold">Edit</span>
                                </button>
                                ...
                                <div class="flex justify-between items-center border-b border-slate-50 pb-2">
                                    ...
                                    <span class="text-xs font-bold text-slate-800" id="detail-orientation">Vertical /
                                        1 Sisi</span>
                                </div>
                                <div class="flex justify-between items-center text-indigo-600 pt-1">
                                    <span class="text-xs font-bold flex items-center gap-2"><i
                                            class="fa-solid fa-wand-sparkles"></i> AI Score</span>
                            ...
                            <div
                                class="bg-white p-5 rounded-xl border border-slate-100 flex-1 flex flex-col h-full min-h-[350px]">
                                <h3
                                    class="font-bold text-xs text-slate-500 uppercase tracking-wide mb-4 flex items-center gap-2">
                        ...
                        <div class="flex flex-col gap-5">
                            ...
                            <div
                                class="bg-white p-5 rounded-xl border border-pink-100 shadow-sm relative overflow-hidden group hover:border-pink-200 transition h-[160px] flex flex-col justify-center">
                                <div
                                    class="absolute right-0 top-0 w-16 h-16 bg-pink-50 rounded-bl-full -mr-4 -mt-4 transition group-hover:bg-pink-100">
                                </div>
                                <div class="relative z-10">
                                    <div class="flex items-center gap-2 mb-2">
                                        <i class="fa-solid fa-award text-pink-500 text-lg"></i>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase">TOTAL IMPRESSIONS
                                            (MO)</span>
                                    </div>
                                    <h2 class="text-4xl font-extrabold text-slate-800 tracking-tight"
                                        id="detail-impressions">0</h2>
                            ...
                            <!-- 2. AUDIENCE PROFILE (Below Impressions) -->
                            <div
                                class="bg-white p-5 rounded-xl border border-slate-100 h-[225px] flex flex-col justify-center">
                                <h3
                                    class="font-bold text-xs text-slate-500 uppercase tracking-wide mb-4 flex items-center gap-2">
                                ...
                                <div
                                    class="bg-slate-50 p-3 rounded-lg flex justify-between items-center mb-5 border border-slate-100">
                                    <span class="text-slate-500 font-medium text-xs">Dominant Age</span>
                                    <span
                                        class="bg-white px-3 py-1 rounded-md font-bold text-slate-800 border border-slate-200 shadow-sm text-xs"
                                        id="detail-dominant-age">18-24 Thn</span>
                                </div>

                                <!-- Gender Bar -->
                                <div
                                    class="mb-1.5 flex justify-between text-[9px] text-slate-400 font-bold uppercase tracking-wider">
                            ...
                            <div class="bg-white p-5 rounded-xl border border-slate-100 h-[350px] flex flex-col">
                                <h3 class="font-bold text-xs text-slate-500 uppercase tracking-wide mb-2">VEHICLE
                                    DISTRIBUTION</h3>
                    ...
                    <div
                        class="bg-white p-8 rounded-xl border border-slate-100 w-full overflow-y-auto custom-scrollbar mt-5 min-h-[180px] flex flex-col justify-center">
                        <h3 class="font-bold text-xs text-slate-500 uppercase tracking-wide mb-6 border-b pb-4">VEHICLE
                            DATA DETAIL</h3>
    ...
    <script>
        ...
        function renderFallback(container) {
            container.innerHTML =
                '<span class="text-xs text-slate-400 italic pl-1">Tidak ada lokasi Terdekat utama dalam radius 2.5 km.</span>';
        }
        ...
        async function fetchRealPlaces(item, containerId, displayMode) {
            ...
            const targetTypes = ['point_of_interest', 'establishment', 'school', 'shopping_mall', 'hospital', 'bank',
                'local_government_office', 'finance', 'place_of_worship'
            ];
            ...
            service.nearbySearch(request, (results, status) => {
                ...
                const processResults = (res) => {
                    return res.map(p => {
                        ...
                        const lat2 = typeof placeLoc.lat === 'function' ? placeLoc.lat() :
                            parseFloat(placeLoc.lat);
                        const lon2 = typeof placeLoc.lng === 'function' ? placeLoc.lng() :
                            parseFloat(placeLoc.lng);

                        const distKm = haversineDistance(lat1, lon1, lat2, lon2);
                        p._realDistance = distKm;
                        p.dist = distKm.toFixed(1) + ' km';
                        return p;
                    }).filter(p => p !== null).sort((a, b) => a._realDistance - b
                    ._realDistance); // Prioritize Nearest
                };
                ...
                if (status === google.maps.places.PlacesServiceStatus.OK && results.length > 0) {
                    ...
                    validPlaces = candidates.sort((a, b) => (b.user_ratings_total || 0) - (a
                ...
                const finalize = () => {
                    ...
                    if (validPlaces.length < maxItems) {
                        ...
                        const fallbacks = [{
                                name: `Pusat Bisnis ${item.city ? item.city.name : ''}`,
                                types: ['office'],
                                dist: '0.8 km'
                            },
                            {
                            ...
                            {
                                name: `Akses Jalan Utama`,
                                types: ['route'],
                                dist: '0.2 km'
                            },
                            {
                                name: `Pemukiman Warga`,
                                types: ['neighborhood'],
                                dist: '0.5 km'
                            },
                            {
                ...
                if (validPlaces.length < maxItems) {
                    service.textSearch({
                        ...
                        if (stat2 === google.maps.places.PlacesServiceStatus.OK && res2.length > 0) {
                            ...
                            expanded.forEach(p => {
                                if (validPlaces.length < maxItems && !validPlaces.find(v => v
                                        .place_id === p.place_id)) {
                                    validPlaces.push(p);
                                }
                            });
                        ...
                        if (validPlaces.length < maxItems) {
                            service.textSearch({
                                ...
                                if (stat3 === google.maps.places.PlacesServiceStatus.OK && res3
                                    .length > 0) {
                                    const wide = processResults(res3).filter(p => p
                                        ._realDistance <= 26.0);
                                    wide.forEach(p => {
                                        if (validPlaces.length < maxItems && !
                                            validPlaces.find(v => v.place_id === p
                                                .place_id)) {
                                            validPlaces.push(p);
                                        }
                                    });
        ...
        function handleMapFilterChange() {
            ...
            if (provinceFilter.value) {
                ...
                cityFilter.innerHTML = '<option value="">Semua Kota</option>' +
                    citiesInProvince.map(c => `<option value="${c}" ${c === currentCity ? 'selected' : ''}>${c}</option>`)
                    .join('');
    ...
    <script>
        ...
        function openAnalysisView(id) {
            ...
            if (item.image2) {
                imgSrc = item.image2.startsWith('http') ? item.image2 : 'https://internal.yousee-indonesia.com/' + item
                    .image2;
            } else if (item.image1) {
                imgSrc = item.image1.startsWith('http') ? item.image1 : 'https://internal.yousee-indonesia.com/' + item
                    .image1;
            } else if (item.image) {
        ...
        function toggleEditMode() {
            ...
            if (!isEditingLocation) {
                ...
                // Replace with Inputs
                // Area Name (Text)
                areaNameEl.innerHTML =
                    `<input type="text" id="input-area-name" value="${escapeHtml(areaNameEl.innerText.trim())}" class="w-full text-sm font-bold text-slate-700 border-b-2 border-blue-400 focus:outline-none bg-transparent py-1">`;

                // Lat/Lng (Number/Text)
                latEl.innerHTML =
                    `<input type="text" id="input-lat" value="${latEl.innerText.trim()}" class="w-full text-xs font-mono font-bold text-slate-600 border-b-2 border-blue-400 focus:outline-none bg-transparent py-1">`;
                lngEl.innerHTML =
                    `<input type="text" id="input-lng" value="${lngEl.innerText.trim()}" class="w-full text-xs font-mono font-bold text-slate-600 border-b-2 border-blue-400 focus:outline-none bg-transparent py-1">`;
                ...
                if (typeEl) {
                    ...
                    let uniqueTypes = [];
                    if (window.mapData) {
                        uniqueTypes = [...new Set(window.mapData.map(item => item.type ? item.type.name : null).filter(
                            Boolean))].sort();
                    }
                    ...
                    let options = uniqueTypes.map(t =>
                        `<option value="${t}" ${t === currentVal ? 'selected' : ''}>${t}</option>`).join('');
                ...
                if (sizeEl) {
                    ...
                    sizeEl.innerHTML =
                        `<input type="text" id="input-size" value="${sizeEl.innerText.trim()}" class="w-full text-right text-xs font-bold text-slate-800 border-b-2 border-blue-400 focus:outline-none bg-transparent py-0.5">`;
                }
                if (orientationEl) {
                    orientationEl.dataset.original = orientationEl.innerText;
                    orientationEl.innerHTML =
                        `<input type="text" id="input-orientation" value="${orientationEl.innerText.trim()}" class="w-full text-right text-xs font-bold text-slate-800 border-b-2 border-blue-400 focus:outline-none bg-transparent py-0.5">`;
                }
                ...
                if (btn) {
                    ...
                    cancelBtn.className =
                        'absolute top-1 right-1/2 mr-1 text-red-500 hover:text-red-700 transition p-1 z-10 flex items-center justify-center h-6 w-6 rounded-full bg-white border border-slate-200 shadow-sm';
        ...
        function saveLocationDataLocal() {
            ...
            if (btn) {
                ...
                btn.innerHTML =
                '<i class="fa-solid fa-check"></i> <span class="text-xs font-bold">Tersimpan (Lokal)</span>';
        ...
        async function saveLocationData() {
            ...
            if (btn) {
                btn.innerHTML =
                    '<i class="fa-solid fa-spinner fa-spin"></i> <span class="text-xs font-bold">Menyimpan...</span>';
                btn.disabled = true;
            }
            ...
            try {
                ...
                if (result.status === 200 || result.success || result.code === 200) {
                    ...
                    if (btn) {
                        ...
                        btn.classList.add('text-green-500');
                        btn.innerHTML =
                            '<i class="fa-solid fa-check"></i> <span class="text-xs font-bold">Tersimpan</span>';
                        setTimeout(() => {
                            btn.classList.remove('text-green-500');
                            btn.innerHTML =
                                '<i class="fa-solid fa-pen-to-square"></i> <span class="text-xs font-bold">Edit</span>';
                        ...
                        chart.data.datasets.forEach((dataset, i) => {
                            ...
                            meta.data.forEach((element, index) => {
                                ...
                                if (val > 5) { // Only label if > 5% to avoid clutter
                                    ...
                                    // Use centroid logic
                                    const model = element;
                                    const midAngle = model.startAngle + (model.endAngle -
                                        model.startAngle) / 2;
                                    const midRadius = (model.innerRadius + model
                                        .outerRadius) / 2;
                                    ...
                                    // Draw Value (Clearer)
                                    const countStr = count >= 1000 ? (count / 1000).toFixed(
                                        1) + 'k' : count;
        ...
        function downloadVisual() {
            ...
            const img1 = item.image1 ? (item.image1.startsWith('http') ? item.image1 :
                'https://internal.yousee-indonesia.com/' + item.image1) : null;
            const img2 = item.image2 ? (item.image2.startsWith('http') ? item.image2 :
                'https://internal.yousee-indonesia.com/' + item.image2) : null;
                    ...
                    htmlToImage.toPng(ghost, {
                            ...
                            filter: (node) => {
                                if (node.tagName === 'LINK' && node.rel === 'stylesheet') {
                                    if (node.href && (node.href.includes('fonts.googleapis.com') || node

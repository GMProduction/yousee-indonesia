...
<head>
    ...
    <style>
        body { font-family: 'Outfit', sans-serif; -webkit-print-color-adjust: exact; }
        .bg-gradient-violet { background: linear-gradient(135deg, #ede9fe 0%, #f3e8ff 100%); }
        .text-violet-main { color: #7c3aed; }
        .bg-violet-main { background-color: #7c3aed; }
        .card-shadow { box-shadow: 0 4px 20px -2px rgba(124, 58, 237, 0.1); }
        .map-container { height: 350px; width: 100%; border-radius: 1rem; overflow: hidden; position: relative; }
...
<body class="bg-slate-50 text-slate-800 p-8 min-h-screen">
    ...
    <div id="capture-target" class="max-w-[1000px] mx-auto bg-white rounded-3xl shadow-xl overflow-hidden card-shadow">
        ...
        <!-- HEADER -->
        <!-- HEADER -->
        <div class="bg-gradient-to-r from-violet-50 via-purple-50 to-white p-8 border-b border-violet-100 flex justify-between items-start">
            <div class="flex flex-col">
                <img src="https://www.yousee-indonesia.com/images/local/logo-yousee2.png" onerror="this.src='https://placehold.co/120x40?text=Yousee'" class="h-16 mb-2" alt="Logo">
            ...
            <div class="col-span-5 bg-gradient-violet p-8 border-r border-violet-100 relative overflow-hidden">
                ...
                <div class="space-y-5 relative z-10">
                    <div>
                        ...
                        <div class="bg-white/80 backdrop-blur px-4 py-3 rounded-xl border border-violet-200/50 shadow-sm min-h-[72px] flex flex-col justify-center">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <div class="bg-white/60 px-3 py-2 rounded-lg border border-violet-100">
                                <span class="font-mono text-xs font-semibold text-slate-600">{{ number_format($item->latitude, 6) }}</span>
                            </div>
                        </div>
                        <div>
                            <label class="text-[10px] uppercase font-bold text-violet-400 block mb-1">Longitude</label>
                            <div class="bg-white/60 px-3 py-2 rounded-lg border border-violet-100">
                                <span class="font-mono text-xs font-semibold text-slate-600">{{ number_format($item->longitude, 6) }}</span>
                            </div>
                        </div>
                    </div>
                     ...
                     <div class="pt-4">
                        <div class="bg-white rounded-2xl p-5 shadow-sm border border-violet-100 mb-4">
                            ...
                            <p class="text-[10px] text-green-600 font-bold mt-1 bg-green-50 inline-block px-2 py-0.5 rounded-full">
                     ...
                     <div class="bg-white/50 rounded-xl p-4 border border-violet-100 mt-4">
                        <table class="w-full text-xs">
                            ...
                            <tr class="border-b border-violet-100/50">
                                ...
                                <td class="py-2 font-bold text-right">{{ $item->orientation ?? 'Vertical' }} / {{ $item->side ?? 1 }} Sisi</td>
                            </tr>
                             <tr>
                    ...
                    <div>
                        ...
                        <div class="bg-slate-50 p-3 rounded-lg">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-xs font-bold text-slate-800 bg-white px-2 py-0.5 rounded shadow-sm" id="aud-age">Loading...</span>
                            </div>
                            <!-- Gender Bar -->
                            <div class="mt-3">
    ...
    <script>
             ...
             service.textSearch(request, (results, status) => {
                 if (status === google.maps.places.PlacesServiceStatus.OK && results.length > 0) {
                     ...
                     list.innerHTML = validPlaces.map(p => {
                         ...
                         if(t.includes('school') || t.includes('university')) { icon='fa-graduation-cap'; color='text-blue-500'; }
                         else if(t.includes('shopping_mall')) { icon='fa-bag-shopping'; color='text-pink-500'; }
                         else if(t.includes('hospital')) { icon='fa-hospital'; color='text-red-500'; }
                         else if(t.includes('bank') || t.includes('finance')) { icon='fa-building-columns'; color='text-green-600'; }
                         else if(t.includes('train_station')) { icon='fa-subway'; color='text-orange-500'; }
                     ...
                     list.innerHTML = `<span class="text-xs text-slate-400 italic">No specific strategic locations data available via API.</span>`;
                 }
             });
        }
        ...
        function calculateSmartProfileImpl(item, returnTraffic = false) {
             ...
             if(item.type.toLowerCase().includes('videotron')) { base += 0.5; trafficEst += 5000; }
             const area = item.width * item.height;
             if(area > 50) { base += 0.5; trafficEst += 2000; }
             if(area > 100) { base += 0.5; trafficEst += 5000; }
             
             // Location Keywords
             const text = (item.address + ' ' + item.name).toLowerCase();
             if(text.includes('tol') || text.includes('highway')) { base += 1.0; trafficEst += 20000; }
             if(text.includes('kota') || text.includes('alun')) { base += 0.8; trafficEst += 10000; }
        ...
        function renderVehicleChart(trafficVolume) {
            ...
            const data = [
                { label: 'Mobil', count: countCar, pct: pCar, color: 'bg-blue-500' },
                { label: 'Motor', count: countMotor, pct: pMotor, color: 'bg-green-500' },
                { label: 'Bus/Truk', count: countBus, pct: pBus, color: 'bg-amber-500' }
            ];

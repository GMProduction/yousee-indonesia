...
class TitikController extends Controller
{
    ...
    {
        $item = Item::with(['city.province', 'type'])->where('slug', $slug)->whereNull('deleted_at')->firstOrFail();
        $titik = Item::with(['city.province', 'type'])
            ->where('isShow', '=', true)
    ...
    /**
     * On-Demand Traffic Check (Safe logic)
     */
    public function checkTraffic($id)
    {
        ...
        return response()->json([
            'score' => 1000,
            'traffic' => $item->trafic > 0 ? $item->trafic : 15000,
            'source' => 'local_db',
            'message' => 'Data traffic dimuat secara lokal'
        ]);
    }

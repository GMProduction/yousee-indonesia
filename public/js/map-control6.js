...
var IMG_BASE = (location.protocol === 'https:' 
...
function normalizeImageUrl(raw) {
  ...
  if (/^https?:\/\//i.test(raw)) {
    ...
    return {
      primary: (isHttp && location.protocol === 'https:') ? raw.replace('http://','https://') : raw,
  ...
  if (raw.startsWith('//')) {
    return {
      primary: (location.protocol === 'https:' ? 'https:' : 'http:') + raw,
  ...
  return {
    primary: (joined.startsWith('http://') && location.protocol === 'https:')
      ? joined.replace('http://','https://') : joined,
    alt: joined.startsWith('https://') ? joined.replace('https://','http://')
...
async function fetchImageFromScraper(slug) {
  try {
    if (!slug) return null;
    var res = await fetch('/scraper.php?slug=' + encodeURIComponent(slug), { cache: 'no-store' });
...
async function generateGoogleMapData() {
  try {
    ...
    var filteredPayload = payload.filter(function (item) {
      var a = item.latitude, b = item.longitude;
...
function updateListTitik(titik) {
  ...
  paginatedItems.forEach(function (d) {
    ...
    listTitikContainer.append(
      ...
      escapeHtml(d.type && d.type.name || '-') + '</div>' +
      '  <div class="article-content"><div class="article-wrapper">' +
      '    <p class="title mt-2">' + escapeHtml(d.city && d.city.province && d.city.province.name || '') + '</p>' +
      '    <p class="time">' + escapeHtml(d.city && d.city.name || '') + '</p>' +
      '    <p class="alamat">' + escapeHtml(d.address || '') + '</p>' +
      '    <hr>' +
      '  </div></div>' +
      '</a>'
    );
  ...
  if (currentPage > 1) paginationContainer.append('<a href="#" class="prev-next" id="prev-page">Prev</a>');

  var isMobile = window.innerWidth <= 540;
  var pageStart = isMobile ? Math.max(1, currentPage - 2) : Math.max(1, currentPage - 4);
  var pageEnd   = isMobile ? Math.min(totalPages, pageStart + 2) : Math.min(totalPages, pageStart + 4);

  for (let i = pageStart; i <= pageEnd; i++) {
    const pageItem = $('<a href="#" class="page-link ' + (i === currentPage ? 'active' : '') + '">' + i + '</a>');
    pageItem.on('click', function (e) { e.preventDefault(); currentPage = i; updateListTitik(titik); });
    paginationContainer.append(pageItem);
  }
  if (currentPage < totalPages) paginationContainer.append('<a href="#" class="prev-next" id="next-page">Next</a>');

  $('#prev-page').on('click', function (e) { e.preventDefault(); if (currentPage > 1) { currentPage--; updateListTitik(titik); } });
  $('#next-page').on('click', function (e) { e.preventDefault(); if (currentPage < totalPages) { currentPage++; updateListTitik(titik); } });
}
  ...
  return new Promise(function (resolve) {
    ...
    payload.forEach(function (v, k) {
      var marker = new google.maps.Marker({
        ...
        map: map_container,
        icon: v['type'] && v['type']['icon'] ? v['type']['icon'] : undefined,
        title: v['name']
      });
      ...
      marker.addListener('click', function () {
        infowindow.open({ anchor: marker, map: map_container, shouldFocus: false });

        google.maps.event.addListenerOnce(infowindow, 'domready', function () {
          (async function () {
            var imgEl   = document.getElementById('iw-img-' + k);
            var loader  = document.getElementById('iw-loader-' + k);
            if (!imgEl) return;
            ...
            imgEl.addEventListener('load', function () {
              ...
              if (wrap && this.naturalWidth && this.naturalHeight) {
                wrap.style.aspectRatio = this.naturalWidth + ' / ' + this.naturalHeight;
              }
            });

            var finalUrl = null;
            try { if (v.slug) finalUrl = await fetchImageFromScraper(v.slug); } catch(e){}
            if (!finalUrl) finalUrl = getFallbackImageFromPayload(v);

            if (finalUrl) imgEl.src = finalUrl;
            else if (loader) loader.innerText = 'Gambar tidak tersedia';
          })();
          ...
          if (btn) {
            btn.addEventListener('click', function () {
              ...
              var slug = v.slug || '';
              if (typeof window.addToCart === 'function') window.addToCart(id, address, slug);
...
async function generateSingleGoogleMapData(id) {
  try {
    ...
    new google.maps.Marker({
      ...
      icon: payload['type'] && payload['type']['icon'] ? payload['type']['icon'] : undefined,
      title: payload['name']
    });
...
function generateDetail(data) {
  ...
  $('#link-gbr1').attr('href', f1 || '#'); $('#dwnld-gbr1').attr({ href: f1 || '#', download: f1 || '' });
  $('#link-gbr2').attr('href', f2 || '#'); $('#dwnld-gbr2').attr({ href: f2 || '#', download: f2 || '' });
  $('#link-gbr3').attr('href', f3 || '#'); $('#dwnld-gbr3').attr({ href: f3 || '#', download: f3 || '' });

  var picPhone = (data['vendor_all']['picPhone'] || '').split('/')[0].split(' ').join('');
  if (picPhone && picPhone[0] === '0') picPhone = '62' + picPhone.substring(1);
  var text = 'Apakah ' + data['type']['name'] + ' yang berlokasi di ' + (data['city']['name'] || '') +
    ' ' + (data['address'] || '') + ' ' + (data['location'] || '') + ' tersedia ?';
